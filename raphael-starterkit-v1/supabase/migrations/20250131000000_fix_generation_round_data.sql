...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE tc.table_schema = 'public'
          AND tc.table_name = 'generated_names'
          AND tc.constraint_name = 'check_generation_round_positive'
    ) THEN
        ALTER TABLE public.generated_names 
...
-- Note: we avoid a partial index with a subquery predicate because
-- PostgreSQL disallows subqueries in index predicates. Also, this
-- condition would be redundant due to the foreign key on batch_id.
-- An index on batch_id already exists (generated_names_batch_id_idx).
